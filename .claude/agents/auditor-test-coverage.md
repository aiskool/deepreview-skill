...
- **Untested modules**: a source file in scope with no corresponding
  test file (look for `test_*`, `*_test.*`, `*.test.*`, `*.spec.*`,
  `tests/`, `__tests__/`, `spec/`). The whole module is a coverage
  hole.
...
6. Write findings to `output_path`:
   ...
   ```json
   {
     ...
     "category": "untested-module|untested-branch|boundary|name-mismatch|flaky|over-mocked",
